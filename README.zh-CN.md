...
AStack 是一个可组合框架，通过"一切皆组件"的理念简化 AI 应用的开发。基于响应式数据流原理，通过事件驱动的执行实现各种 AI 模型、工具和自定义业务逻辑之间的无缝集成。
...
- **响应式数据流**：基于 RxJS Observable 的发布-订阅模式，实现组件间解耦通信
- **事件驱动执行**：基于流程基础编程范式的数据驱动自动触发
- **无锁并发**：多个数据包同时流经管道，无需手动锁管理
...
- **更好的可扩展性**：数据流天然支持并发、事件驱动的处理
- **降低复杂度**：避免控制流框架的臃肿（有些框架单文件超过 11000 行）

AStack 的响应式数据流基于流程基础编程（Flow-Based Programming）原理，使用 RxJS Observables，组件通过发布-订阅模式通信。这实现了：
- **事件驱动执行**：组件响应数据事件，而非控制信号
- **无锁并发**：多个数据包同时流动，无需显式加锁
- **解耦通信**：组件无需了解彼此的实现细节
...
| 功能 | AStack | Haystack |
|---------|--------|----------|
| **核心范式** | 数据流（FBP）- 响应式、事件驱动 | 控制流 - 命令式、while 循环调度 |
| **执行模型** | RxJS Observable 流与发布-订阅 | 优先队列与基于 dict 的数据传递 |
| **并发性** | 无锁并发执行（FBP 范式） | 手动异步管理与优先级调度 |
| **数据传递** | 通过端口连接的响应式流 | 组件间的 dict 深拷贝 |
...
- **数据流 vs 控制流**：AStack 的响应式数据流范式与 LLM 作为无状态生成器的特性自然契合，而 Haystack 使用传统的 while 循环调度控制流模式
- **并发模型**：AStack 通过 FBP 的事件驱动模型实现无锁并发；Haystack 通过优先队列和手动异步处理管理并发
- **性能优化**：AStack 构建一次拓扑并复用；Haystack 在每次执行时进行拓扑排序
- **类型系统**：AStack 通过 TypeScript 提供编译时安全性；Haystack 提供运行时检查与可选验证

每个框架都有其优势，并针对不同的用例进行了优化。Haystack 在 RAG 应用和文档处理方面表现出色，拥有成熟的 Python 生态系统和丰富的调试工具，而 AStack 专为 TypeScript/JavaScript 环境设计，强调响应式数据流、事件驱动执行，以及自然契合 Agent 框架需求的数据流范式。AStack 还提供完整的中文文档，使其对中文开发者特别友好。
