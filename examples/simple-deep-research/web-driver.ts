...
export default class WebDriverComponent extends Component {
  ...
  private async initBrowser(): Promise<{ browser: Browser, context: BrowserContext, page: Page }> {
    if (!this.browser) {
      try {
        ...
        throw new Error(`初始化浏览器失败: ${error.message || '未知错误'}`);
      }
    }
  ...
  _transform($i: any, $o: any) {
    ...
    $i('searchQuery').receive(async (searchTerm: string) => {
      ...
      try {
        ...
        // 检查结果格式
        console.log(`[WebDriver] 搜索结果类型: ${typeof results}, 是否数组: ${Array.isArray(results)}`);
        if (results.length > 0) {
    ...
    $i('url').receive((url: string) => {
      ...
      console.log(`[WebDriver] 当前队列长度: ${urlQueue.length}, 处理状态: ${isProcessingUrl ? '正在处理' : '空闲'}`);
  ...
  private async searchBaidu(searchTerm: string): Promise<SearchResultItem[]> {
    ...
    try {
      ...
      $('.result.c-container, .c-container').each((index, element) => {
        ...
        const snippet = $(element).find('.c-abstract, .content-abstract, .c-abstract-content').text().trim();
  ...
  private async fetchPageContent(url: string): Promise<string> {
    ...
    try {
      ...
      // 移除无关内容
      $('script, style, nav, footer, header, .header, .footer, .nav, .menu, .ad, .ads, .advertisement').remove();
      ...
      const errorMessage = `获取页面内容失败: ${error.message || error}`;
